...
function check_new_repo_mane () {
  if [ "$NUMBER_PARAMETERS" -eq  "0" ] || [ "${NEW_REPO_NAME}" = "" ]; then
...
function install_gh () {
  ...
  is_brew_installed
  if [[ $? -eq $FALSE ]] ; then 
        echo '😱 BREW 🍺 is not installed. Just a sec I am working on it...'
        echo " "
    install_brew;
  else
...
function install_gh_if_necessary () {
  is_gh_installed
  if [[ $? -eq $FALSE ]] ; then 
    echo "🙀 GH is not installed. Just a sec I am fixing it..."
    echo " "
    install_gh;
  else
